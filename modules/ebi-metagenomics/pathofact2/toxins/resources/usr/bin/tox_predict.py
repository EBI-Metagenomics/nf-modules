...
def preprocess_sequences(sequences, kmer_sizes, vectorizer_dir, num_cpus):
    def process_kmer(k):
        ...
        seqs = [seq for _, seq in sequences]  # Extract only the sequences for vectorization
...
def main():
    ...
    parser.add_argument('-cpu', '--cpus', type=int, default=-1, help='Number of CPUs to use for parallel processing. Default is -1 (use all available CPUs)')

    args = parser.parse_args()

    num_cpus = args.cpus if args.cpus > 0 else -1  # Use all available CPUs if non-positive value is provided
    ...
    X_new = preprocess_sequences(sequences_with_ids, args.kmers, args.vectorizer_dir, num_cpus)
    ...
    with open(args.output, 'w') as fout:
        fout.write(f"Sequence\tPrediction\tProbability\n")
