...
def count2(aa_set, sequence):
    ...
    length = len(sequence)
    number = len(indices)
    cutoff_nums = [1, math.floor(0.25 * number), math.floor(0.50 * number), math.floor(0.75 * number), number]
    cutoff_nums = [max(i, 1) for i in cutoff_nums]
    return [(indices[cutoff - 1] / length) * 100 if cutoff <= number else 0 for cutoff in cutoff_nums]
...
def ctd_d_for_sequence(name, sequence, group1, group2, group3, properties):
    ...
    encodings = []
    for prop in properties:
        encodings.extend(count2(group1[prop], sequence) + count2(group2[prop], sequence) + count2(group3[prop], sequence))
    return [name] + encodings
...
def ctd_t_for_sequence(name, sequence, group1, group2, group3, properties):
    ...
    encodings = []
    for prop in properties:
        c1221 = sum(1 for pair in aa_pairs if (pair[0] in group1[prop] and pair[1] in group2[prop]) or (pair[0] in group2[prop] and pair[1] in group1[prop]))
        c1331 = sum(1 for pair in aa_pairs if (pair[0] in group1[prop] and pair[1] in group3[prop]) or (pair[0] in group3[prop] and pair[1] in group1[prop]))
        c2332 = sum(1 for pair in aa_pairs if (pair[0] in group2[prop] and pair[1] in group3[prop]) or (pair[0] in group3[prop] and pair[1] in group2[prop]))
        total_pairs = len(aa_pairs)
        encodings.extend([c1221 / total_pairs, c1331 / total_pairs, c2332 / total_pairs])
    return [name] + encodings
...
def dpc(fastas, num_cpus):
    AA = 'ACDEFGHIKLMNPQRSTVWY'
    di_peptides = [aa1 + aa2 for aa1 in AA for aa2 in AA]
...
def ctdc(fastas, group1, group2, group3, properties, num_cpus):
    ...
    with ProcessPoolExecutor(max_workers=num_cpus) as executor:
        future_to_sequence = {
            executor.submit(ctdc_for_sequence, name, sequence, group1, group2, group3, properties): (name, sequence)
            for name, sequence in fastas
        }
        for future in as_completed(future_to_sequence):
            encodings.append(future.result())
...
def ctd_d(fastas, group1, group2, group3, properties, num_cpus):
    ...
    with ProcessPoolExecutor(max_workers=num_cpus) as executor:
        future_to_sequence = {
            executor.submit(ctd_d_for_sequence, name, sequence, group1, group2, group3, properties): (name, sequence)
            for name, sequence in fastas
        }
        for future in as_completed(future_to_sequence):
            encodings.append(future.result())
    headers = ['#'] + [f"{p}.{g}.residue{d}" for p in properties for g in ('1', '2', '3') for d in ['0', '25', '50', '75', '100']]
    df = pd.DataFrame(encodings, columns=headers)
    return df
...
def ctd_t(fastas, group1, group2, group3, properties, num_cpus):
    ...
    with ProcessPoolExecutor(max_workers=num_cpus) as executor:
        future_to_sequence = {
            executor.submit(ctd_t_for_sequence, name, sequence, group1, group2, group3, properties): (name, sequence)
            for name, sequence in fastas
        }
        for future in as_completed(future_to_sequence):
            encodings.append(future.result())
    headers = ['#'] + [f"{p}.{tr}" for p in properties for tr in ('Tr1221', 'Tr1331', 'Tr2332')]
    df = pd.DataFrame(encodings, columns=headers)
    return df
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', metavar='model', help='The model file (SAV format)', required=True)
    parser.add_argument('--cpus', metavar='cpus', type=int, help='number of CPUs', default=1)
    parser.add_argument('--outfile', metavar='outfile', help='The output file (TSV format)', required=True)
    ...
    df = reduce(lambda left, right: pd.merge(left, right, on=['#'], how='outer'), data_frames)
    ...
    prediction = pd.DataFrame({
        'Sequence': df['#'],
        'Prediction': rfc_predict,
        'Probability': rfc_probs
    })
